...
OUT="./runs/v13_review_fixed_45ep_antisat"
mkdir -p "$OUT"

# Anti-saturation run after tuned report:
# - lower signal-op prior
# - lower gate bias/floors
# - more dropout/diversity
# - softer planner temperatures
# - slower LR
...
python sequential_matrix_cells_speechcommands_v13_categorized_signal_bus_builder.py \
  ...
  --late-gate-floor-end 0.03 \
  --late-gate-floor-decay-start 6 \
  --late-gate-floor-decay-epochs 12 \
  --late-write-target 0.28 \
  --late-dyn-gate-target 0.08 \
  ...
  --lambda-memory-diversity 0.008 \
  --lambda-operator-entropy-per-block 0.0012 \
  --lambda-block-diversity 0.040 \
  --lambda-route-diversity 0.032 \
  --lambda-adapter-diversity 0.010 \
  --lambda-phase-role 0.014 \
  --lambda-global-skip-penalty 0.003 \
  --lambda-late-write 0.006 \
  --lambda-late-dyn-gate 0.001 \
  --lambda-weak-class-late-read 0.014 \
  --lambda-role-diversity 0.012 \
  --lambda-role-anchor 0.008 \
  --lambda-category-diversity 0.008 \
  --lambda-category-entropy 0.0016 \
  --decomp-every 20 \
  --speed-log \
...
echo "Push: bash commands/push_run_report.sh $OUT v13_review_fixed_45ep_antisat 'Add anti-saturation v13 review fixed 45ep report'"
