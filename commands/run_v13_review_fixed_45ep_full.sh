...
OUT="./runs/v13_review_fixed_45ep"
mkdir -p "$OUT"
...
python tools/export_program_pseudocode.py "$OUT/seq_analysis_epoch_045.json" --out "$OUT/program_pseudocode.md" || true
python tools/analyze_v13_health_report.py "$OUT" || true

echo "DONE: $OUT"
echo "Review: cat $OUT/health_diagnosis.md"
echo "Push: bash commands/push_run_report.sh $OUT v13_review_fixed_45ep 'Add v13 review fixed 45ep report'"
