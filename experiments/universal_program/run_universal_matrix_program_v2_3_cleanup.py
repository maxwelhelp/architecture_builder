...
from run_universal_matrix_program_v1 import (  # noqa: E402
    BASE_OPS, CATS, HFHiddenTask, devtype, losses, seed_all, write_json,
)
...
def _safe_get(xs, idx, default=None):
    try:
        return xs[idx]
    except Exception:
        return default
...
def _cat_consistency(cat_vals: List[float], op_vals: List[float], cat_op: List[List[float]]) -> float:
    if not cat_vals or not op_vals or not cat_op:
        return 0.0
    c = torch.tensor(cat_vals, dtype=torch.float32)
    o = torch.tensor(op_vals, dtype=torch.float32)
    m = torch.tensor(cat_op, dtype=torch.float32)
    p = min(o.numel(), m.shape[1])
    c = c / c.sum().clamp_min(1e-8)
    o = o[:p] / o[:p].sum().clamp_min(1e-8)
    desired = (c @ m[:, :p]).clamp_min(1e-8)
    desired = desired / desired.sum().clamp_min(1e-8)
    return float((desired * o).sum().item())
...
class CleanupPlannerController:
    """v2.3 controller.

    Fixes after v2.2:
    - category/op consistency is measured and can be repaired by changing weak category priors;
    - prune is no longer permanently disabled by over-strict thresholds;
    - action selection still uses global planner attention over all active blocks/steps;
    - actions are evaluated with pending reward/cooldown.
    """
    ...
    def __init__(self, args) -> None:
        ...
        self.last_scores: Dict[str, float] = {
            "add_step": 0.0, "add_depth": 0.0, "add_width": 0.0,
            "add_primitive": 0.0, "prune_step": 0.0, "repair_category": 0.0,
        }
    ...
    def _head_pressure(self, val: Dict[str, float], aux: Dict[str, Any]) -> List[float]:
        ...
        return [delta_pain * float(_safe_get(delta_read, i, 0.0)) + 0.5 * norm_pain * float(_safe_get(norm_read, i, 0.0)) for i in range(len(block_map))]
    ...
    def _program_features(self, model: UniversalProgramV21, val: Dict[str, float], aux: Dict[str, Any], grad_health: Any) -> List[Dict[str, Any]]:
        block_map = aux.get("block_index_map", [])
        pressures = self._head_pressure(val, aux)
        op_steps = aux.get("operator_mix_by_layer_block_step", [])
        cat_steps = aux.get("category_mix_by_layer_block_step", [])
        ...
        cat_op = model.core.cat_op.detach().cpu().tolist()
        feats = []
        for global_idx, bm in enumerate(block_map):
            layer = int(bm["layer_id"]); block = int(bm["block_id"]); order = int(bm["order_pos"])
            ...
            b_cats = _safe_get(_safe_get(cat_steps, order, []), block, []) or []
            ...
            consistency_vals = []
            for s in range(min(len(b_ops), len(b_cats))):
                consistency_vals.append(_cat_consistency(b_cats[s], b_ops[s], cat_op))
            consistency = sum(consistency_vals) / max(1, len(consistency_vals))
            block_cos_i = float(_safe_get(block_cos, order, 0.0))
            planner_logit = (
                3.0 * pressure + 0.55 * op_entropy + 0.25 * route_mean + 0.20 * grad_mean
                + 0.15 * block_cos_i - 0.08 * max(0, step_count - 1) + 0.12 * (1.0 - consistency)
            )
            feats.append({
                "global_index": global_idx, "layer": layer, "block": block, "order": order,
                "step_count": step_count, "pressure": pressure, "op_entropy": op_entropy,
                "gate": gate_mean, "route_entropy": route_mean, "grad": grad_mean,
                "block_cos": block_cos_i, "category_consistency": consistency,
    ...
    def _action_scores(self, model: UniversalProgramV21, val: Dict[str, float], feat: Dict[str, Any], aux: Dict[str, Any]) -> Dict[str, float]:
        ...
        pressure = float(feat["pressure"]); entropy = float(feat["op_entropy"]); grad = float(feat["grad"])
        route = float(feat["route_entropy"]); gate = float(feat["gate"]); block_cos = float(feat["block_cos"])
        attn = float(feat.get("planner_attention", 0.0)); consistency = float(feat.get("category_consistency", 0.0))
        low_entropy = max(0.0, 0.45 - entropy)
        scores = {
            "add_step": 0.42 * attn + 0.32 * entropy + 0.18 * pressure + 0.13 * grad + 0.06 * route - 0.20 * max(0, step_count - 1) - self._cool("add_step", layer, block, step_count),
            "add_depth": 0.55 * delta_gap + 0.28 * pressure + 0.16 * route + 0.12 * grad - 0.06 * max(0, active_layers - 4) - self._cool("add_depth", layer, block),
            "add_width": 0.36 * block_cos + 0.32 * pressure + 0.22 * attn - 0.10 * max(0, active_blocks - 4) - self._cool("add_width", layer, block),
            "add_primitive": 0.33 * low_entropy + 0.18 * grad + 0.08 * delta_gap - 0.12 * max(0, active_prims - len(BASE_OPS)) - self._cool("add_primitive", layer, block),
            "prune_step": 0.20 * max(0, step_count - 1) + 0.10 * max(0.0, 0.35 - gate) + 0.08 * max(0.0, 0.30 - consistency) - 0.16 * pressure - 0.10 * grad - self._cool("prune_step", layer, block),
            "repair_category": 0.38 * max(0.0, 0.45 - consistency) + 0.10 * pressure + 0.05 * grad - self._cool("repair_category", layer, block),
        }
    ...
    def _choose(self, model: UniversalProgramV21, val: Dict[str, float], aux: Dict[str, Any], grad_health: Any) -> Tuple[str, Dict[str, Any], Dict[str, float], List[Dict[str, Any]]]:
        feats = self._program_features(model, val, aux, grad_health)
        if not feats:
            return "soft_explore", {"layer": 0, "block": 0, "order": 0, "step_count": 1, "category_consistency": 0.0}, self.last_scores, []
        ranked = sorted(feats, key=lambda f: f.get("planner_attention", 0.0), reverse=True)
        best_action = "soft_explore"; best_feat = ranked[0]; best_scores = self.last_scores; best_val = -999.0
        for feat in ranked[:min(10, len(ranked))]:
            scores = self._action_scores(model, val, feat, aux)
            action = max(scores, key=scores.get)
            if scores[action] > best_val:
                best_val = scores[action]; best_action = action; best_feat = feat; best_scores = scores
        top = [{k: f[k] for k in ["layer", "block", "step_count", "pressure", "op_entropy", "category_consistency", "grad", "gate", "route_entropy", "planner_attention"] if k in f} for f in ranked[:10]]
        if best_val < -0.05:
            best_action = "soft_explore"
        return best_action, best_feat, best_scores, top

    def _repair_category(self, model: UniversalProgramV21, feat: Dict[str, Any], aux: Dict[str, Any]) -> Tuple[bool, str]:
        layer = int(feat.get("layer", 0)); block = int(feat.get("block", 0)); order = int(feat.get("order", 0))
        op_steps = aux.get("operator_mix_by_layer_block_step", [])
        ops = _safe_get(_safe_get(op_steps, order, []), block, []) or []
        cat_op = model.core.cat_op.detach().cpu()
        if not ops:
            return False, "no_step_ops_for_category_repair"
        with torch.no_grad():
            for s, op_vals in enumerate(ops[:int(model.core.active_steps[layer, block].item())]):
                op = torch.tensor(op_vals[:cat_op.shape[1]], dtype=torch.float32)
                op = op / op.sum().clamp_min(1e-8)
                cat_score = cat_op[:, :op.numel()] @ op
                best_cat = int(torch.argmax(cat_score).item())
                model.core.cat_prior[layer, block, s, best_cat] += 0.08
        return True, f"repaired_category_prior_L{layer}B{block}_steps_{len(ops)}"
    ...
    def _do_action(self, model: UniversalProgramV21, action: str, feat: Dict[str, Any], aux: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if action == "repair_category":
            return self._repair_category(model, feat, aux)
    ...
    def step(self, model: UniversalProgramV21, epoch: int, val: Dict[str, float], aux: Dict[str, Any], active: List[str], grad_health: Any) -> CleanupEvent:
        self._tick()
        ...
        scores = self.last_scores; top = self.last_top; consistency = 0.0
        ...
        if self.pending is not None and epoch - int(self.pending["epoch"]) >= self.pending_epochs:
            ...
            rec = dict(self.pending); rec.update({"epoch_eval": epoch, "score_after": score, "reward": pr})
            ...
            if pr < -self.min_delta and self.pending.get("state_before") is not None:
                ...
                return CleanupEvent(epoch, mode, action, score, self.absolute_best, reward, False, -1, -1, -1, reason, detail, **self._scores_for_event(scores), category_consistency=0.0, planner_attention_top=top)
        ...
        if score > self.significant_best + self.min_delta:
            ...
            if self.bad >= self.rollback and self.absolute_state is not None:
                model.load_arch_state(self.absolute_state); model.core.soft_action("reduce_priors")
                ...
                target_layer = int(feat.get("layer", -1)); target_block = int(feat.get("block", -1)); target_step = int(feat.get("step_count", -1)); consistency = float(feat.get("category_consistency", 0.0))
                if not ok:
                    action = "soft_explore"; model.core.soft_action("explore", target_layer); detail = "fallback_soft_explore"
                if action.startswith("add") or action.startswith("prune") or action == "repair_category":
                    self.pending = {"epoch": epoch, "action": action, "layer": target_layer, "block": target_block, "step": target_step, "score_before": score, "state_before": state_before, "detail": detail, "scores": scores, "category_consistency": consistency}
                mode = "GROW" if action.startswith("add") or action.startswith("prune") or action == "repair_category" else "EXPLORE"
                reason = "plateau_cleanup_planner"
                self.flat = 0
        self.prev = score
        return CleanupEvent(epoch, mode, action, score, self.absolute_best, reward, accepted, target_layer, target_block, target_step, reason, detail, **self._scores_for_event(scores), category_consistency=consistency, planner_attention_top=top)
    ...
    def _scores_for_event(self, scores: Dict[str, float]) -> Dict[str, float]:
        return {
            ...
            "repair_category_score": float(scores.get("repair_category", 0.0)),
        }
...
def run(args) -> None:
    ...
    for ep in range(1, args.epochs + 1):
        ...
        print(f"ep {ep:03d}/{args.epochs} score={ev.score:.5f} best={ev.best_score:.5f} mode={ev.mode} action={ev.action} target=L{ev.target_layer}B{ev.target_block}S{ev.target_step} scores(step={ev.step_score:.3f},depth={ev.depth_score:.3f},width={ev.width_score:.3f},prim={ev.primitive_score:.3f},prune={ev.prune_score:.3f},repair={ev.repair_category_score:.3f},cons={ev.category_consistency:.3f}) {ev.growth_detail} " + " ".join([f"{n}:mse={val.get(n+'_mse',0):.4g},cos={val.get(n+'_cos',0):.4f}" for n in active]), flush=True)
    write_json(out / "final_report.json", {"architecture": "universal_matrix_program_v2_3_cleanup", "final_arch_state": model.export_arch_state(), "action_history": ctrl.history, "args": vars(args)})
...
def parser():
    ...
    p.add_argument("--out-dir", default="./runs/universal_matrix_program_v2_3_cleanup_hf_45ep")
    return p
